...
import {
  SettingsHighlighterSettings,
  DEFAULT_SETTINGS,
  SettingsHighlighterSettingTab,
} from "./settings";
...
export default class SettingsHighlighterPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.communityRibbonEl = this.addRibbonIcon(
      "puzzle",
      "Community plugins",
      () => {
  ...
  private injectRibbonStyles() {
    ...
    style.id = id;
    style.textContent =
      ".sh-ribbon-hidden.sh-ribbon-hidden { display: none !important; }";
  ...
  private repositionRibbonIcons(): boolean {
    ...
    if (this.communityRibbonEl)
      sidebarEl.insertBefore(this.communityRibbonEl, anchor);
  ...
  onunload() {
    ...
    if (modal) {
      this.setHeadingCollapsed(
        this.findSectionHeading(modal, "Core plugins"),
        false
      );
      this.setHeadingCollapsed(
        this.findSectionHeading(modal, "Community plugins"),
        false
      );
      modal
        .querySelectorAll<HTMLElement>(".sh-chevron")
  ...
  updateRibbonVisibility() {
    if (this.communityRibbonEl) {
      this.communityRibbonEl.classList.toggle(
        "sh-ribbon-hidden",
        !this.settings.showCommunityRibbonButton
      );
    }
    if (this.coreRibbonEl) {
      this.coreRibbonEl.classList.toggle(
        "sh-ribbon-hidden",
        !this.settings.showCoreRibbonButton
      );
    }
    if (this.flaskRibbonEl) {
      this.flaskRibbonEl.classList.toggle(
        "sh-ribbon-hidden",
        !this.settings.showFlaskButton
      );
    }
  }
  ...
  private attachTabObserver(modal: HTMLElement) {
    ...
    const navContainer =
      modal.querySelector(".vertical-tab-header") ??
      ...
      if (active) {
        ...
        if (!heading.querySelector(".sh-chevron")) {
          ...
          chevron.style.cssText =
            "display:inline-block;margin-left:6px;transition:transform 0.15s ease;opacity:0.5;font-size:0.9em;transform:rotate(90deg);";
          chevron.textContent = "›";
  ...
  private highlightActiveTab(modal: HTMLElement) {
    ...
    const tabId = (this.app as any).setting?.activeTab?.id as
      | string
      | undefined;
    const isCommunityPlugin = !!(
      tabId && (this.app as any).plugins?.plugins?.[tabId]
    );
    const isCorePlugin = !!(
      tabId && (this.app as any).internalPlugins?.plugins?.[tabId]
    );
    ...
    if (isPlugin) {
      const parentNavItem = this.findOptionsNavItem(
        modal,
        isCommunityPlugin ? "Community plugins" : "Core plugins"
      );
      if (parentNavItem) {
  ...
  private updateSectionCollapse(
    ...
    const isOnCoreList =
      tabId === "core-plugins" || activeText === "Core plugins";
    const isOnCommunityList =
      tabId === "community-plugins" || activeText === "Community plugins";

    const coreHeading = this.findSectionHeading(modal, "Core plugins");
    const communityHeading = this.findSectionHeading(
      modal,
      "Community plugins"
    );
  ...
  private findSectionHeading(
    ...
    const byAttr = modal.querySelector<HTMLElement>(
      `[data-sh-label="${label}"]`
    );
    if (byAttr) return byAttr;

    const groups = Array.from(
      modal.querySelectorAll<HTMLElement>(".vertical-tab-header-group")
    );
    for (const group of groups) {
      const title = group.querySelector<HTMLElement>(
        ".vertical-tab-header-group-title"
      );
      if (title?.textContent?.trim() === label) return title;
    }

    const flatHeadings = Array.from(
      modal.querySelectorAll<HTMLElement>(".vertical-tab-nav-item-heading")
    );
  ...
  private getSectionNavItems(heading: HTMLElement): HTMLElement[] {
    ...
    if (group) {
      ...
      if (wrapper)
        return Array.from(
          wrapper.querySelectorAll<HTMLElement>(".vertical-tab-nav-item")
        );
      return Array.from(
        group.querySelectorAll<HTMLElement>(".vertical-tab-nav-item")
      );
    }
    ...
    while (el) {
      ...
      if (el.classList.contains("vertical-tab-nav-item"))
  ...
  private findOptionsNavItem(
    modal: HTMLElement,
    label: string
  ): HTMLElement | null {
    const navItems = Array.from(
      modal.querySelectorAll<HTMLElement>(".vertical-tab-nav-item")
    );
  ...
  private clearHighlightsIn(modal: HTMLElement) {
    const targets =
      ".vertical-tab-nav-item, .vertical-tab-header-group-title, .vertical-tab-nav-item-heading";
