...
class TodayFolderHighlightPlugin extends Plugin {
	async onload() {
		...
		this.injectStyles();
		this.registerEvent(this.app.workspace.on("layout-change", () => this.applyHighlight()));
	...
	onunload() {
		document.querySelectorAll("." + CSS_CLASS).forEach((el) => el.classList.remove(CSS_CLASS));
	...
	injectStyles() {
		...
		styleEl.textContent = [
			".nav-folder-title." + CSS_CLASS + " { color: " + darkColor + " !important; font-weight: 700 !important; text-decoration: underline !important; }",
			".theme-light .nav-folder-title." + CSS_CLASS + " { color: " + lightColor + " !important; }",
		].join("\n");
	}
...
class TodayFolderHighlightSettingTab extends PluginSettingTab {
	...
	display() {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.useAccentColor).onChange(async (value) => {
		...
		if (!this.plugin.settings.useAccentColor) {
			...
			new Setting(containerEl)
				...
				.addColorPicker((picker) =>
					picker.setValue(this.plugin.settings.customColorLight).onChange(async (value) => {
