...
function makeAlarmId() { return 'a_' + Math.random().toString(36).slice(2, 10); }
function makeTimerId() { return 't_' + Math.random().toString(36).slice(2, 10); }
...
const CLOCK_TOKENS = [
    ...
    { token: 'hh', fn: (d) => { const h = d.getHours() % 12 || 12; return String(h).padStart(2, '0'); } },
...
const TIMER_TOKENS = [
    ...
    { token: 'mm', fn: (s) => String(Math.floor((s % 3600) / 60)).padStart(2, '0') },
...
function renderDurationSteppers(parentEl, initialSeconds, options) {
    ...
    return { row, hourInput, minInput, secInput, getSeconds: read, initH, initM, initS };
}
...
class DurationModal extends obsidian.Modal {
    ...
    onOpen() {
        ...
        [nameInput, stepper.hourInput, stepper.minInput, stepper.secInput].forEach((inp) => {
        ...
        window.setTimeout(() => {
            const target = stepper.initH > 0 ? stepper.hourInput
                : stepper.initM > 0 ? stepper.minInput
                : stepper.secInput;
            target.focus();
            target.select();
        }, 0);
...
class NotificationModal extends obsidian.Modal {
    ...
    onOpen() {
        applyModalFit(this);
        const { contentEl } = this;
        const heading = contentEl.createEl('h3', { text: `${this.icon} ${this.label}` });
        heading.style.textAlign = 'center';
        const row = contentEl.createEl('div');
...
class TimersListModal extends obsidian.Modal {
    ...
    renderContent() {
        ...
        const addBtn = contentEl.createEl('button', { text: '＋ Add timer', cls: 'mod-cta' });
        ...
        for (const t of sorted) {
            ...
            const pauseLabel = t.mode === 'paused' ? '▶' : t.mode === 'alarming' ? '✓' : '⏸';
            const pauseTitle = t.mode === 'paused' ? 'Resume'
                : t.mode === 'alarming' ? 'Dismiss'
                : 'Pause';
...
class StatusBarClockSettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        if (s.clockEnabled) {
            new obsidian.Setting(containerEl)
                .setName('Time format')
                .setDesc('Tokens: H (hour), HH (padded 24h), h (12h), hh (padded 12h), mm (minutes), ss (seconds), A (AM/PM), a (am/pm). Example: h:mm:ss A')
                .addText(text => text
        ...
        new obsidian.Setting(containerEl)
            ...
            .setDesc('Show the alarm status-bar item and fire scheduled alarms. Disabling silences all alarms but keeps definitions.')
            .addToggle(toggle => toggle
                .setValue(s.alarmsEnabled)
                .onChange(async (value) => {
        ...
        if (s.alarmsEnabled) {
            new obsidian.Setting(containerEl)
                ...
                .setDesc('Show the alarm status-bar item. Disabling hides the item but keeps alarms firing.')
                .addToggle(toggle => toggle
                    .setValue(s.alarmShowInStatusBar)
                    .onChange(async (value) => {
            ...
            const addAlarmBtn = addAlarmRow.createEl('button', { text: '＋ Add alarm', cls: 'mod-cta' });
            addAlarmBtn.addEventListener('click', () => this.plugin.openAddAlarmModal(() => this.display()));
            ...
            new obsidian.Setting(containerEl).setName('Alarm notifications').setHeading();

            new obsidian.Setting(containerEl)
                .setName('Pulse status bar')
                .setDesc('Gentle 2-second opacity pulse on the alarm item when an alarm fires.')
                .addToggle(toggle => toggle
                    .setValue(s.alarmNotificationPulse)
                    .onChange(async (value) => {
            ...
            const alarmPopupSetting = new obsidian.Setting(containerEl)
                ...
                .setDesc('Notice = sticky toast. Modal = blocking dialog that must be dismissed.');
            ...
            new obsidian.Setting(containerEl)
                ...
                .addToggle(toggle => toggle
                    .setValue(s.alarmNotificationAudio)
                    .onChange(async (value) => {
        ...
        new obsidian.Setting(containerEl)
            ...
            .setDesc('Show the timer status-bar item and allow timer commands. Disabling clears all active timers.')
            .addToggle(toggle => toggle
                .setValue(s.timerEnabled)
                .onChange(async (value) => {
        ...
        if (s.timerEnabled) {
            new obsidian.Setting(containerEl)
                .setName('Show in status bar')
                .setDesc('Show the timer status-bar item. Disabling hides the item but keeps timer commands and ribbon buttons working.')
                .addToggle(toggle => toggle
                    .setValue(s.timerShowInStatusBar)
                    .onChange(async (value) => {
            ...
            new obsidian.Setting(containerEl)
                ...
                .setDesc('Pre-filled placeholder when adding a new timer (e.g. "Pomodoro").')
                .addText(text => text
            ...
            const defaultDurationSetting = new obsidian.Setting(containerEl)
                ...
                .setDesc('Pre-filled into the duration prompt when adding a new timer.');
            const initialDefaultSeconds = parseDuration(s.timerDefaultDuration) || 0;
            renderDurationSteppers(defaultDurationSetting.controlEl, initialDefaultSeconds, {
            ...
            new obsidian.Setting(containerEl)
                .setName('Format type')
                .setDesc('Clock: colon-separated tokens (mm:ss, hh:mm:ss). Duration: letter-suffixed units (1h05m08s).')
                .addDropdown(dd => dd
            ...
            if (s.timerFormatType === 'clock') {
                new obsidian.Setting(containerEl)
                    ...
                    .setDesc('Tokens: H/HH (hours), m/mm (minutes 0–59), M (total minutes), s/ss (seconds 0–59). Example: mm:ss')
                    .addText(text => text
            ...
            new obsidian.Setting(containerEl).setName('Timer notifications').setHeading();

            new obsidian.Setting(containerEl)
                .setName('Pulse status bar')
                .setDesc('Gentle 2-second opacity pulse on the timer item when finished. Subtle to avoid photosensitivity issues.')
                .addToggle(toggle => toggle
                    .setValue(s.timerNotificationPulse)
                    .onChange(async (value) => {
            ...
            new obsidian.Setting(containerEl)
                ...
                .addButton(btn => btn
                    ...
                    .onClick(() => this.plugin.startAudio()))
                .addToggle(toggle => toggle
                    .setValue(s.timerNotificationAudio)
                    .onChange(async (value) => {
    ...
    renderAlarmRow(parentEl, alarm) {
        ...
        del.addEventListener('click', async () => {
            this.plugin.settings.alarms = this.plugin.settings.alarms.filter(a => a.id !== alarm.id);
            await this.plugin.saveSettings();
            this.plugin.renderAlarmStatus();
            this.display();
        });
...
class StatusBarClockPlugin extends obsidian.Plugin {
    async onload() {
        ...
        this.timerAddRibbonEl = this.addRibbonIcon('clock-plus', 'Add timer', () => this.promptAndStart());
        this.timerListRibbonEl = this.addRibbonIcon('clock-alert', 'View active timers', () => this.openTimersListModal());
        this.alarmListRibbonEl = this.addRibbonIcon('alarm-clock', 'View alarms', () => {
            this.app.setting.open();
            this.app.setting.openTabById('status-bar-clock');
        });
        this.alarmAddRibbonEl = this.addRibbonIcon('alarm-clock-plus', 'Add alarm', () => this.openAddAlarmModal());
        this.flaskRibbonEl = this.addRibbonIcon('flask-conical', 'Status Bar Clock settings', () => {
            this.app.setting.open();
            this.app.setting.openTabById('status-bar-clock');
        });
        ...
        this.alarmDisplayInterval = window.setInterval(() => this.renderAlarmStatus(), 30000);
        ...
        this.addCommand({ id: 'timer-add', name: 'Add timer', callback: () => this.promptAndStart() });
        this.addCommand({ id: 'timer-list', name: 'View active timers', callback: () => this.openTimersListModal() });
        this.addCommand({ id: 'timer-pause-resume', name: 'Pause / resume primary timer', callback: () => this.toggleSimplePauseResume() });
        this.addCommand({ id: 'timer-reset', name: 'Clear all timers', callback: () => this.clearAllTimers() });
        this.addCommand({ id: 'alarm-add', name: 'Add alarm', callback: () => this.openAddAlarmModal() });
    }
    ...
    updateRibbonVisibility() {
        ...
        if (this.flaskRibbonEl) this.flaskRibbonEl.classList.toggle('sbc-ribbon-hidden', !s.showFlaskButton);
        if (this.timerAddRibbonEl) this.timerAddRibbonEl.classList.toggle('sbc-ribbon-hidden', !timerAddVisible);
        if (this.timerListRibbonEl) this.timerListRibbonEl.classList.toggle('sbc-ribbon-hidden', !timerListVisible);
        if (this.alarmListRibbonEl) this.alarmListRibbonEl.classList.toggle('sbc-ribbon-hidden', !alarmListVisible);
        if (this.alarmAddRibbonEl) this.alarmAddRibbonEl.classList.toggle('sbc-ribbon-hidden', !alarmAddVisible);
    }

    applyTimerVisibility() {
        if (this.timerEl) {
            const hidden = !this.settings.timerEnabled || !this.settings.timerShowInStatusBar;
            this.timerEl.classList.toggle('sbc-timer-hidden', hidden);
        }
    }

    applyClockVisibility() {
        if (this.statusBarItem) this.statusBarItem.classList.toggle('sbc-clock-hidden', !this.settings.clockEnabled);
    }

    applyAlarmVisibility() {
        if (!this.alarmEl) return;
        const hidden = !this.settings.alarmsEnabled || !this.settings.alarmShowInStatusBar;
    ...
    repositionRibbonIcons() {
        ...
        if (this.timerAddRibbonEl) sidebarEl.insertBefore(this.timerAddRibbonEl, anchor);
        if (this.timerListRibbonEl) sidebarEl.insertBefore(this.timerListRibbonEl, anchor);
        if (this.alarmListRibbonEl) sidebarEl.insertBefore(this.alarmListRibbonEl, anchor);
        if (this.alarmAddRibbonEl) sidebarEl.insertBefore(this.alarmAddRibbonEl, anchor);
    ...
    formatTimerValue(seconds, initialSeconds) {
        if (this.settings.timerFormatType === 'duration') {
            return formatTimerDuration(seconds, this.settings.timerDurationLeadingZeros, initialSeconds || seconds);
        }
    ...
    primaryTimer() {
        ...
        const running = ts.filter(t => t.mode === 'running').sort((a, b) => a.remainingMs - b.remainingMs)[0];
        if (running) return running;
        return ts.filter(t => t.mode === 'paused').sort((a, b) => a.remainingMs - b.remainingMs)[0] || null;
    }
    ...
    renderTimer() {
        ...
        this.timerEl.setText(name ? `⏱ ${name} ${formatted}${suffix}` : `⏱ ${formatted}${suffix}`);
        this.timerEl.toggleClass('is-paused', primary.mode === 'paused');
        this.timerEl.toggleClass('is-alarming', primary.mode === 'alarming');
        this.timerEl.toggleClass('is-pulsing', primary.mode === 'alarming' && !!this.settings.timerNotificationPulse);
        this.timerEl.setAttr('aria-label', `${count} active timer${count > 1 ? 's' : ''} · click to manage`);
    }
    ...
    promptAndStart() {
        ...
        const defaultName = (this.settings.timerDefaultName || '').trim();
        new DurationModal(this.app, defaultDuration, async (seconds, rawValue, name) => {
    ...
    fireTimerExpiry(timer) {
        ...
        if (this.settings.timerNotificationModal) {
            new NotificationModal(this.app, () => this.dismissTimerById(timer.id), label, '⏱').open();
        }
    }
    ...
    renderAlarmStatus() {
        ...
        if (next) {
            ...
            else if (mode === 'next-time-name') text = name ? `⏰ ${name} ${timeStr}` : `⏰ ${timeStr}`;
        }
    ...
    fireAlarm(alarm) {
        ...
        if (this.settings.alarmNotificationModal) {
            new NotificationModal(this.app, () => this.clearAlarmFiringState(), label, '⏰').open();
        }
    ...
    clearAlarmFiringState() {
        if (this.alarmEl) this.alarmEl.classList.remove('is-alarming', 'is-pulsing');
    ...
    async loadSettings() {
        ...
        this.settings.alarms = this.settings.alarms.map(a => ({
            ...
            days: (a && Array.isArray(a.days) && a.days.length === 7) ? a.days.map(d => !!d) : DEFAULT_ALARM_DAYS.slice(),
