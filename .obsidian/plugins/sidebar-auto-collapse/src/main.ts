...
function readVaults(currentPath: string): VaultEntry[] {
    try {
        ...
        return Object.entries(vaults).map(([id, v]: [string, any]) => ({
            id,
...
export default class AutoCollapseSidebarPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.vaultRibbonEl = this.addRibbonIcon('vault', 'Switch vault', () => this.openVaultSwitcher());

        this.pluginSettingsRibbonEl = this.addRibbonIcon('flask-conical', 'Sidebar Auto-Collapse settings', () => {
            (this.app as any).setting.open();
            (this.app as any).setting.openTabById('sidebar-auto-collapse');
        });

        this.obsidianSettingsRibbonEl = this.addRibbonIcon('settings', 'Open settings', () => {
            (this.app as any).setting.open();
        });
        ...
        this.app.workspace.onLayoutReady(() => {
            ...
            const leftEl = this.app.workspace.containerEl.querySelector('.mod-left-split') as HTMLElement | null;
            if (leftEl) {
                this.registerDomEvent(leftEl, 'mousemove', () => {
                    if (this.settings.enabled && this.settings.resetOnActivity && this.collapseTimer !== null) {
                        this.startTimer();
                    }
                });
    ...
    private repositionRibbonIcons(): boolean {
        ...
        if (this.pluginSettingsRibbonEl) sidebarEl.appendChild(this.pluginSettingsRibbonEl);
        if (this.obsidianSettingsRibbonEl) sidebarEl.appendChild(this.obsidianSettingsRibbonEl);
        return true;
    }

    applyRibbonVisibility() {
        if (this.vaultRibbonEl) this.vaultRibbonEl.classList.toggle('sac-ribbon-hidden', !this.settings.showVaultSwitcher);
        if (this.pluginSettingsRibbonEl) this.pluginSettingsRibbonEl.classList.toggle('sac-ribbon-hidden', !this.settings.showPluginSettings);
        if (this.obsidianSettingsRibbonEl) this.obsidianSettingsRibbonEl.classList.toggle('sac-ribbon-hidden', !this.settings.showObsidianSettings);
    }
    ...
    private injectRibbonStyles() {
        ...
        style.id = id;
        style.textContent = '.sac-ribbon-hidden.sac-ribbon-hidden { display: none !important; }';
    ...
    private openVaultSwitcher() {
        ...
        const currentPath: string = adapter.basePath || adapter.getBasePath?.() || '';
        ...
        new VaultSwitcherModal(this.app, vaults, (v) => {
