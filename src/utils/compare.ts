...
import { arrayAvg, percentDiff } from "./calc";
...
const avgHex = (list: CompareItem[]): CompareItem => {
  ...
  list.forEach(({ hexA, hexB }) => {
    ...
    const rA = hexA.RGB?.red ?? 0, rB = hexB.RGB?.red ?? 0;
    const gA = hexA.RGB?.green ?? 0, gB = hexB.RGB?.green ?? 0;
    const bA = hexA.RGB?.blue ?? 0, bB = hexB.RGB?.blue ?? 0;
    const hA = hexA.HSL?.hue ?? 0, hB = hexB.HSL?.hue ?? 0;
    const sA = hexA.HSL?.sat ?? 0, sB = hexB.HSL?.sat ?? 0;
    const lA = hexA.HSL?.lum ?? 0, lB = hexB.HSL?.lum ?? 0;
    ...
    redPercents.push(percentDiff(rA, rB));
    greenPercents.push(percentDiff(gA, gB));
    bluePercents.push(percentDiff(bA, bB));
    huePercents.push(percentDiff(hA, hB));
    satPercents.push(percentDiff(sA, sB));
    lumPercents.push(percentDiff(lA, lB));
  });
